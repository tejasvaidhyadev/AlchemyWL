...
def train_model(model, train_dataloader, val_dataloader, test_dataloader, gen_dataloader, voc1, voc2, device, config, logger, min_train_loss=float('inf'), min_val_loss=float('inf'), min_test_loss=float('inf'), 
	...
	for epoch in range(1, config.epochs + 1):
		...
		if config.test_set and (config.test_always or epoch >= config.epochs - (config.eval_last_n - 1)):
			test_loss_epoch, test_acc_epoch = run_validation(config=config, model=model, dataloader=test_dataloader, disp_tok='TEST', voc1=voc1, voc2=voc2, device=device, logger=logger, epoch_num = epoch, validation = False)
		...
		if config.gen_set and (config.gen_always or epoch >= config.epochs - (config.eval_last_n - 1)):
			gen_loss_epoch, gen_acc_epoch = run_validation(config=config, model=model, dataloader=gen_dataloader, disp_tok='GEN', voc1=voc1, voc2=voc2, device=device, logger=logger, epoch_num = epoch, validation = False)
			wandb.log({"generalization loss per epoch": gen_loss_epoch})
			wandb.log({"generalization accuracy": gen_acc_epoch, "epoch": epoch})
		else:
		...
		wandb.log({"max generalization accuracy": max_gen_acc})
